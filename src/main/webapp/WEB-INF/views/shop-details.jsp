...
.thumb-img {
	width: 100%;
	height: 100%;
	object-fit: cover; /* Hiển thị ảnh không bị méo */
	cursor: pointer;
	border: 2px solid transparent;
}

.thumb-img:hover, .thumb-img.active {
	border-color: red; /* Hiệu ứng viền khi chọn ảnh */
}
...
.white { border-color: white; color: black; background-color: white; }
.gray { border-color: gray; color: gray; }
...
.white:hover { background-color: white; color: black; border-color: black; }
.gray:hover { background-color: gray; color: white; }
.pink:hover { background-color: pink; color: white; }
.gold:hover { background-color: gold; color: white; }
...
document.addEventListener("DOMContentLoaded", function () { 
	...
	<section class="shop-details mt-5">
		<div class="container">
			<div class="row">
				<!-- Hình ảnh sản phẩm -->
				<div class="col-lg-5">
					<div class="product-images">
						<div class="main-image">
							<img id="mainProductImage"
								src="<c:url value='/assets/img/product/${detailProduct[3][0]}'/>"
								alt="" class="img-fluid rounded shadow">
						</div>
						<div class="thumb-images d-flex mt-3 overflow-auto">
							<c:forEach items="${detailProduct[3]}" var="item">
								<div class="thumb-item me-2">
									<img src="<c:url value='/assets/img/product/${item}'/>" alt=""
										class="img-thumbnail thumb-img" onclick="changeImage(this)">
								</div>
							</c:forEach>
						</div>
					</div>
				...
				<div class="col-lg-7">
					<div class="product-info">
						<h2 class="product-title">${detailProduct[1]}</h2>
						<div class="rating d-flex align-items-center">
							<span class="stars"> <i class="fa fa-star text-warning"></i>
								<i class="fa fa-star text-warning"></i> <i
								class="fa fa-star text-warning"></i> <i
								class="fa fa-star text-warning"></i> <i class="fa fa-star-o"></i>
							</span> <span class="ms-2">3,8k Đánh giá</span>
						</div>
						<div class="product-price mt-2">
							<span class="text-danger fw-bold h3"> <fmt:formatNumber
									value="${detailProduct[2]}" pattern="#,###" /> &#8363;
							</span> <span class="text-muted text-decoration-line-through ms-2">
								<fmt:formatNumber value="${detailProduct[2] * 1.3}"
									pattern="#,###" /> &#8363;
							</span> <span class="badge bg-danger ms-2">-30%</span>
						</div>

						<div class="product-options mt-4">
							<label class="fw-bold">Màu sắc:</label>
							<div class="color-options d-flex gap-2">
    <button class="btn color-btn black">Đen</button>
    <button class="btn color-btn white">Trắng</button>
    <button class="btn color-btn gray">Xám</button>
    <button class="btn color-btn pink">Hồng</button>
    <button class="btn color-btn gold">Vàng</button>
    <button class="btn color-btn red">Đỏ</button>
</div>
    ...
    <input id="quantity" type="number" value="1" min="1" max="10" class="form-control" style="width:10%;">
</div>


						<div class="product-buttons mt-4">
							<button onclick="addToCart(${detailProduct[0]})"
								class="btn btn-danger px-4 py-2"> <i class="bi bi-cart3 me-3" style="font-size: 20px;"></i>Thêm vào giỏ hàng</button>
							<button class="btn btn-warning px-4 py-2">Mua ngay</button>
						</div>

						<div class="product-meta mt-4">
							<p>
								<strong>Sản phẩm:</strong> ${detailProduct[0]}
							</p>
							<p>
								<strong>Danh mục:</strong> ${detailProduct[4].name}
							</p>
							<p>
								<strong>Vận chuyển:</strong> Nhận hàng từ 3 - 5 ngày
							</p>
						
							<p>
								<strong>Chính sách đổi trả:</strong> Đổi trả trong 7 ngày nếu có
								lỗi từ nhà sản xuất
							</p>
						</div>
					</div>
...
function changeImage(imgElement) {
	...
	<section class="related spad mt-5">
		<div class="container">
			<div class="row">
				<div class="col-lg-12">
					<h5 class="related-title" style="color: gray;">
			...
			<div class="row row-cols-1 row-cols-md-3 row-cols-lg-5 g-3">
				<c:forEach items="${relatedProducts}" var="item">
					<div class="col">
						<div class="product__item border rounded shadow-sm p-2 product-card">
							<div class="product__item__pic position-relative"
								onclick="detail(${item.key})">
								<img
									src="<c:url value='/assets/img/product/${item.value[3][0]}'/>"
									class="img-fluid rounded" alt=""> <span
									class="badge bg-danger position-absolute top-0 start-0">-30%</span>
								<span
									class="badge bg-warning text-dark position-absolute top-0 end-0">Yêu
									thích</span>
							...
							<div class="product__item__text mt-2">
								...
								<div class="d-flex justify-content-between align-items-center">
									<span class="text-muted text-decoration-line-through"> <fmt:formatNumber
											value="${item.value[1] * 1.3}" pattern="#,###" /> &#8363;
